...
const DocumentPreview = forwardRef<any, DocumentPreviewProps>(({
    ...
    return `
      ...
      <body>
        ...
        <div class="center small">
          ...
          <p class="break-word">${accessKey}</p>
  ...
  const shareContent = () => {
    let content = `Documento Fiscal ${number} - Paulo Cell - Valor: ${formatCurrency(value + calculateTaxes().total)} - Emitido em: ${date.toLocaleString('pt-BR')}`;
    
    // Se houver descrição, adiciona uma versão resumida (primeiros 100 caracteres)
    if (description) {

  ...
  const generateDocumentHTML = () => {
    ...
    <html>
      ...
      <body>
        ...
        <p class="center bold">${getDocumentTypeLabel().toUpperCase()}
        <table class="small">
          <tr>
            <td><b>Documento:</b></td>
        ...
        <div class="center small">
          ...
          <p class="break-word">${document.access_key}</p>
...
`;
  ...
  const handleDelete = async () => {
    try {
      ...
      // Verificar se o usuário confirma a exclusão
      if (!confirm(`Tem certeza que deseja excluir permanentemente o documento ${document.number}?`)) {
        return;
      }
    ...
    return `
      ...
      <body>
        ...
        <div class="center small">
          ...
          <p>Consulte pela chave de acesso em:</p>
          <p class="bold">www.nfe.fazenda.gov.br</p>
        </div>
        
        <div class="divider"></div>
        
        <div class="footer">
          Documento emitido em ${new Date().toLocaleDateString('pt-BR')} ${new Date().toLocaleTimeString('pt-BR')}
          <br>
...
`;
  ...
  // Handler para impressão via Bluetooth
  const handleBluetoothPrint = () => {
    // Usar a biblioteca diretamente para conectar à impressora e imprimir
    import('@/lib/bluetooth-printer').then(async module => {
      const { 
      ...
      if (!isBluetoothSupported()) {
        toast({
          ...
          description: "Seu navegador não suporta Bluetooth. Use Chrome ou Edge em um dispositivo com Bluetooth.",
          variant: "destructive"
        });
        return;
      }
      ...
      toast({
        title: "Procurando impressoras...",
        description: "Selecione sua impressora na caixa de diálogo do navegador."
      });
      ...
      try {
        ...
        toast({
          ...
          description: `Conectado à impressora ${printers[0].name}. Imprimindo...`
        });
        ...
        await printText(printContent);
        
        toast({
          title: "Impressão enviada!",
          description: "O documento foi enviado para a impressora com sucesso."
        });
  ...
  // Gerar conteúdo formatado para impressão em impressora térmica Bluetooth
  const generatePrintContent = () => {
    // Formato simplificado para impressora térmica
    const status = document.status === 'authorized' 
      ? 'AUTORIZADO' 
    ...
    const content = 
      ...
      `Data: ${new Date(document.issue_date).toLocaleDateString('pt-BR')}\n` +
      `Cliente: ${document.customer_name}\n` +
      `Status: ${status}\n` +
      `Valor: R$ ${document.total_value.toFixed(2).replace('.', ',')}\n\n` +
      `${document.description ? `Descrição:\n${document.description}\n\n` : ''}` +
      `${document.access_key ? `Chave de Acesso:\n${document.access_key}\n\n` : ''}` +
      `-------------------\n` +
    ...
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          ...
          <DropdownMenuItem onClick={handleBluetoothPrint} className="cursor-pointer">
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="border-t pt-4">
              <div className="flex justify-between">
                ...
                <Button variant="outline" onClick={handleBluetoothPrint}>
                  <Bluetooth className="mr-2 h-4 w-4" />

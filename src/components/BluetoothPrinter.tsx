  ...
  ({ document, children }, ref) => {
    ...
    const printFiscalDocument = async () => {
      try {
        ...
        // Valores e impostos
        content.push({ text: "------------------------", align: "center" });
        content.push({ text: `Subtotal: ${formatCurrency(document.total_value)}` });
        
        // Impostos baseados no tipo de documento
        if (document.type === 'nf') {
          content.push({ text: `ICMS (18%): ${formatCurrency(taxes.icms)}` });
          content.push({ text: `IPI (5%): ${formatCurrency(taxes.ipi)}` });
        } else if (document.type === 'nfce') {
          content.push({ text: `ICMS (18%): ${formatCurrency(taxes.icms)}` });
        } else if (document.type === 'nfs') {
          content.push({ text: `ISS (5%): ${formatCurrency(taxes.iss)}` });
        }
        
        content.push({ text: `Total com impostos: ${formatCurrency(totalWithTaxes)}`, bold: true });

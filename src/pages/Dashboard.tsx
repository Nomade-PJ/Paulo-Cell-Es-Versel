    ...
    const fetchDashboardData = async () => {
      try {
        ...
        const revenueByDay = last7Days.map(day => {
          ...
          const dayRevenue = dayServices.reduce((sum, service) => sum + (service.price || 0), 0);
          ...
          <CardContent>
            ...
            <div className="flex items-center pt-2">
              ...
              <p className={`text-xs ${revenue.percentChange > 0 ? 'text-green-500' : revenue.percentChange < 0 ? 'text-red-500' : 'text-orange-500'}`}>
                {revenue.percentChange > 0 ? '+' : ''}{Math.round(revenue.percentChange || 0)}% em relação ao mês anterior
              </p>
            </div>
          </CardContent>
        </Card>
              ...
              >
                <AreaChart data={revenueChartData}>
                  ...
                  <ChartTooltip
                    content={({ active, payload }) => {
                      if (active && payload?.length) {
                        const value = payload[0] && payload[0].value !== undefined ? payload[0].value : 0;
                        return (
                          <ChartTooltipContent>
